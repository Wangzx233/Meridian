...
func (a *API) handleRunnerWS(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		case "runner.heartbeat":
			...
			go a.publishAndMaybeAssign(context.Background(), payload.RunnerID)
		case "runner.update.response", "runner.shutdown.response", "fs.list.response", "project.files.response", "project.file.read.response", "project.file.write.response", "project.file.upload.response", "project.file.action.response", "project.command.response", "project.terminal.open.response":
