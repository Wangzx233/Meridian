...
func (s *Store) ListServers(ctx context.Context) ([]Server, error) {
	rows, err := s.db.Query(ctx, `SELECT id, name, runner_id, status, last_heartbeat_at, created_at, updated_at FROM servers ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CreateServer(ctx context.Context, in CreateServerInput) (Server, error) {
	...
	row := s.db.QueryRow(ctx, `
		INSERT INTO servers (name, runner_id, status)
		VALUES ($1, $2, 'offline')
		RETURNING id, name, runner_id, status, last_heartbeat_at, created_at, updated_at`,
		in.Name, in.RunnerID)
	return scanServer(row)
}

func (s *Store) GetServer(ctx context.Context, id string) (Server, error) {
	row := s.db.QueryRow(ctx, `SELECT id, name, runner_id, status, last_heartbeat_at, created_at, updated_at FROM servers WHERE id=$1`, id)
	return scanServer(row)
}
...
func (s *Store) PatchServer(ctx context.Context, id string, in PatchServerInput) (Server, error) {
	...
	row := s.db.QueryRow(ctx, `
		UPDATE servers SET name=$2, runner_id=$3, status=$4, updated_at=now()
		WHERE id=$1
		RETURNING id, name, runner_id, status, last_heartbeat_at, created_at, updated_at`,
		id, name, runnerID, status)
	return scanServer(row)
}
